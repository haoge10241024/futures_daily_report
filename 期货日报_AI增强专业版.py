...
def create_report_professional(custom_date_str, symbol, commodity_name, user_description, main_view, 
    ...
    # 获取市场数据，兼容返回4个或5个值的情况
    market_data_result = get_market_trend_data(symbol=symbol, custom_date=custom_date)
    if len(market_data_result) == 5:
        # 新版本：返回5个值（包含market_date）
        market_trend_description, night_trend_description, market_data, _, _ = market_data_result
    else:
        # 旧版本：返回4个值
        market_trend_description, night_trend_description, market_data, _ = market_data_result
...
if st.button("🎨 生成K线图", type="primary"):
    if not full_contract or not commodity_name:
        ...
        with st.spinner("正在生成K线图..."):
            ...
            # 获取市场数据，兼容返回4个或5个值的情况
            market_data_result = get_market_trend_data(full_contract, custom_date)
            if len(market_data_result) == 5:
                # 新版本：返回5个值（包含market_date）
                day_description, night_description, market_data, market_data_dict, _ = market_data_result
            else:
                # 旧版本：返回4个值
                day_description, night_description, market_data, market_data_dict = market_data_result
