...
class EnhancedNewsSearcher:
    ...
    def search_with_serper_api(self, commodity: str, days_back: int = 3, api_key: str = None, target_date=None):
        """使用Serper API进行搜索（支持指定目标日期或日期列表）"""
        if not api_key:
            return []
        
        try:
            # target_date可以是单个日期字符串或日期列表
            ...
            all_news = []
            for target_date_str in dates_to_search:
    ...
    def search_professional_data(self, commodity: str, serper_key: str, target_date=None):
        """搜索专业期货分析数据（8大维度）"""
        try:
            # 如果是日期列表，使用最新的日期；如果是字符串，直接使用；否则使用当前日期
            if isinstance(target_date, list) and len(target_date) > 0:
                target_date_str = target_date[-1]  # 使用最新日期
            elif isinstance(target_date, str):
                target_date_str = target_date
            else:
                target_date_str = datetime.now().strftime('%Y-%m-%d')
            
            # 将 target_date 统一为字符串格式
...
def get_last_trading_day(symbol, report_date, max_days_back=7):
        ...
        try:
            ...
            if not df.empty:
                ...
                if not day_data.empty:
                    print(f"   ✅ 找到交易日：{check_date_str}（向前{days_back}天）")
                    if days_back > 1:
                        weekday = check_date.strftime('%A')
                        weekday_cn = {'Monday': '周一', 'Tuesday': '周二', 'Wednesday': '周三', 
                                    'Thursday': '周四', 'Friday': '周五', 'Saturday': '周六', 'Sunday': '周日'}
                        print(f"   💡 提示：跳过了{days_back-1}个非交易日（最近交易日是{weekday_cn.get(weekday, weekday)}）")
                    return check_date
    ...
    print(f"   ⚠️ 未找到有效交易日（已回溯{max_days_back}天）")
    return None
...
def get_market_trend_data(symbol, custom_date):
    ...
    try:
        ...
        print(f"   正在从akshare获取{symbol}的分钟数据...")
        df = ak.futures_zh_minute_sina(symbol=symbol, period="1")
        
        if df.empty:
            print(f"   ⚠️ akshare未返回任何数据，可能原因：")
            print(f"      - 合约代码{symbol}不存在或格式错误")
            print(f"      - 该合约已到期或尚未上市")
            return "", "", pd.DataFrame(), {}, None
        ...
        if filtered_data.empty:
            print(f"   ⚠️ 指定时间范围内无数据")
            print(f"      可用数据时间范围：{df['datetime'].min()} 至 {df['datetime'].max()}")
            return "", "", pd.DataFrame(), {}, None
        ...
        if day_data.empty:
            print(f"   ⚠️ 日盘时段（{day_start} 至 {day_end}）无数据")
            return "", "", pd.DataFrame(), {}, None
        ...
        print(f"   ❌ 数据获取异常: {type(e).__name__}")
        print(f"   错误详情: {str(e)}")
        if "symbol" in str(e).lower() or "code" in str(e).lower():
            print(f"   💡 提示：请检查合约代码格式（如：CU2501、M2505）")
        elif "network" in str(e).lower() or "timeout" in str(e).lower():
            print(f"   💡 提示：网络连接问题，请检查网络或稍后重试")
...
if st.button("🎨 生成K线图", type="primary"):
    if not full_contract or not commodity_name:
        ...
        with st.spinner("正在生成K线图..."):
            ...
            # 生成资讯日期列表：从交易日到日报日期的所有日期
            if actual_market_date is not None:
                st.session_state.actual_market_date = actual_market_date
                news_date_list = []
                ...
                date_range_str = f"{actual_market_date.strftime('%m-%d')} 至 {custom_date.strftime('%m-%d')}" if actual_market_date != custom_date else custom_date.strftime('%m-%d')
                st.info(f"📅 资讯搜索范围：{date_range_str}（共{len(news_date_list)}天）")
                target_date_param = news_date_list
            else:
                # 如果没有找到交易日，使用默认逻辑
                target_date_param = custom_date_str
            
            # 获取新闻（传入日期列表确保覆盖所有相关日期）
